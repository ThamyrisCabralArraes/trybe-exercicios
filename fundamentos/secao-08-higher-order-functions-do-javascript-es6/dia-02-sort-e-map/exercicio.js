...
// books.forEach((book) => console.log(`${book.name} - ${book.genre} - ${book.author.name} `));

const formatedAuthorNamesBirth = () => {
  const expectedResult = books.map((book) => `${book.author.name} - ${book.author.birthYear}`);
  console.log(expectedResult.sort());
};
